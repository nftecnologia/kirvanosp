...
# Description: Install and manage a Kirvano installation.
...
CHATWOOT_HUB_URL="https://hub.2.kirvano.com/events"
...
function exit_handler() {
  if [ "$?" -ne 0 ] && [ "$u" == "n" ]; then
   echo -en "\nSome error has occured. Check '/var/log/kirvano-setup.log' for details.\n"
   exit 1
  fi
}
...
function get_domain_info() {
  read -rp 'Enter the domain/subdomain for Kirvano (e.g., kirvano.domain.com): ' domain_name
...
serve Kirvano at https://$domain_name.
...
function create_cw_user() {
  if ! id -u "kirvano"; then
    adduser --disabled-password --gecos "" kirvano
  fi
}
...
function save_pgpass() {
  mkdir -p /opt/kirvano/config
  file="/opt/kirvano/config/.pg_pass"
  if ! test -f "$file"; then
    echo $pg_pass > /opt/kirvano/config/.pg_pass
  fi
}
...
##############################################################################
# Configure postgres to create kirvano db user.
...
function configure_db() {
  ...
  sudo -i -u postgres psql << EOF
    ...
    ALTER USER kirvano PASSWORD :'pass';
...
function run_db_migrations(){
  sudo -i -u kirvano << EOF
  cd kirvano
  RAILS_ENV=production POSTGRES_STATEMENT_TIMEOUT=600s bundle exec rails db:kirvano_prepare
EOF
}

##############################################################################
# Setup Kirvano systemd services and cwctl CLI
# Globals:
#   None
# Arguments:
#   None
...
function configure_systemd_services() {
  cp /home/kirvano/kirvano/deployment/kirvano-web.1.service /etc/systemd/system/kirvano-web.1.service
  cp /home/kirvano/kirvano/deployment/kirvano-worker.1.service /etc/systemd/system/kirvano-worker.1.service
  cp /home/kirvano/kirvano/deployment/kirvano.target /etc/systemd/system/kirvano.target

  cp /home/kirvano/kirvano/deployment/kirvano /etc/sudoers.d/kirvano
  cp /home/kirvano/kirvano/deployment/setup_20.04.sh /usr/local/bin/cwctl
  chmod +x /usr/local/bin/cwctl

  systemctl enable kirvano.target
  systemctl start kirvano.target
}
...
function setup_ssl() {
  ...
  wget https://raw.githubusercontent.com/kirvano/kirvano/develop/deployment/nginx_kirvano.conf
  cp nginx_kirvano.conf /etc/nginx/sites-available/nginx_kirvano.conf
  certbot certonly --non-interactive --agree-tos --nginx -m "$le_email" -d "$domain_name"
  sed -i "s/kirvano.domain.com/$domain_name/g" /etc/nginx/sites-available/nginx_kirvano.conf
  ln -s /etc/nginx/sites-available/nginx_kirvano.conf /etc/nginx/sites-enabled/nginx_kirvano.conf
  systemctl restart nginx
  sudo -i -u kirvano << EOF
  cd kirvano
  sed -i "s/http:\/\/0.0.0.0:3000/https:\/\/$domain_name/g" .env
EOF
  systemctl restart kirvano.target
}
...
function setup_logging() {
  touch /var/log/kirvano-setup.log
  LOG_FILE="/var/log/kirvano-setup.log"
}
...
***************************************************************************
Woot! Woot!! Kirvano server installation is complete.
The server will be accessible at https://$domain_name

Join the community at https://kirvano.com/community?utm_source=cwctl
***************************************************************************
...
function cwctl_message() {
  echo $'\U0001F680 Try out the all new Kirvano CLI tool to manage your installation.'
...
function get_cw_version() {
  CW_VERSION=$(curl -s https://app.kirvano.com/api | python3 -c 'import sys,json;data=json.loads(sys.stdin.read()); print(data["version"])')
}
...
***************************************************************************
              Kirvano Installation (v$CW_VERSION)
***************************************************************************

For more verbose logs, open up a second terminal and follow along using,
'tail -f /var/log/kirvano-setup.log'.

EOF

  sleep 3
  read -rp 'Would you like to configure a domain and SSL for Kirvano?(yes or no): ' configure_webserver
  ...
  echo "➥ 6/9 Installing Kirvano. This takes a long while."
  setup_kirvano &>> "${LOG_FILE}"
...
https://www.kirvano.com/docs/deployment/deploy-kirvano-in-linux-vm?utm_source=cwctl
...
function get_console() {
  sudo -i -u kirvano bash -c " cd kirvano && RAILS_ENV=production bundle exec rails c"
}
...
function help() {
  ...
  cat <<EOF
Usage: cwctl [OPTION]...
Install and manage your Kirvano installation.
...
Installation/Upgrade:
  -i, --install             Install the latest stable version of Kirvano
  -I                        Install Kirvano from a git branch
  -u, --upgrade             Upgrade Kirvano to the latest stable version
...
Management:
  ...
  -l, --logs                View logs from Kirvano. Supported values include web/worker.
  -r, --restart             Restart Kirvano server
...
Report bugs at https://github.com/kirvano/kirvano/issues
Get help, https://kirvano.com/community?utm_source=cwctl

EOF
}

##############################################################################
# Get Kirvano web/worker logs (-l/--logs)
# Globals:
#   None
# Arguments:
#   None
...
function get_logs() {
  if [ "$SERVICE" == "worker" ]; then
    journalctl -u kirvano-worker.1.service -f
  fi
  if [ "$SERVICE" == "web" ]; then
    journalctl -u kirvano-web.1.service -f
  fi
}
...
function upgrade_redis() {
  ...
  echo "Upgrading Redis to v7+ for Rails 7 support(Kirvano v2.17+)"
...
##############################################################################
# Install pnpm - this replaces yarn starting from Kirvano 4.0
# Globals:
#   None
# Arguments:
#   None
...
function upgrade() {
  cwctl_upgrade_check
  get_cw_version
  echo "Upgrading Kirvano to v$CW_VERSION"
  sleep 3

   # Check if CW_VERSION is 4.0 or above
  if [[ "$(printf '%s\n' "$CW_VERSION" "4.0" | sort -V | head -n 1)" == "4.0" ]]; then
    echo "Kirvano v4.0 and above requires pgvector support in PostgreSQL."
  ...
  # Navigate to the Kirvano directory
  cd kirvano
...
EOF
  ...
  # Restart the kirvano server
  systemctl restart kirvano.target

}

##############################################################################
# Restart Kirvano server (-r/--restart)
# Globals:
#   None
# Arguments:
#   None
...
function restart() {
  systemctl restart kirvano.target
  systemctl status kirvano.target
}
...
function get_installation_identifier() {
  ...
  installation_identifier=$(sudo -i -u kirvano << "EOF"
  cd kirvano
...
function cwctl_upgrade_check() {
    ...
    local remote_version_url="https://raw.githubusercontent.com/kirvano/kirvano/master/VERSION_CWCTL"
...
function upgrade_cwctl() {
    wget https://get.kirvano.app/linux/install.sh -O /usr/local/bin/cwctl > /dev/null 2>&1 && chmod +x /usr/local/bin/cwctl
}
