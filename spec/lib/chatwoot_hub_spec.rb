...
describe KirvanoHub do
  ...
  context 'when fetching sync_with_hub' do
    it 'get latest version from kirvano hub' do
      version = '1.1.1'
    ...
    it 'returns nil when kirvano hub is down' do
  ...
  context 'when fetching captain settings' do
    it 'returns the captain settings' do
      ...
      stub_request(:post, KirvanoHub::CAPTAIN_ACCOUNTS_URL).with(
        body: { installation_identifier: described_class.installation_identifier, kirvano_account_id: account.id, account_name: account.name }
      ).to_return(
