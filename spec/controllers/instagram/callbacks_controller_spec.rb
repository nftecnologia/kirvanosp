...
RSpec.describe Instagram::CallbacksController do
  ...
  let(:exception_tracker) { instance_double(KirvanoExceptionTracker) }
  ...
  before do
    ...
    allow(controller).to receive(:base_url).and_return('https://app.kirvano.com')
    ...
    allow(KirvanoExceptionTracker).to receive(:new).and_return(exception_tracker)
