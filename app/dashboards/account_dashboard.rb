...
class AccountDashboard < Administrate::BaseDashboard
  ...
  enterprise_attribute_types = if KirvanoApp.enterprise?
                                 attributes = {
                                   limits: AccountLimitsField
                                 }

                                 # Only show manually managed features in Kirvano Cloud deployment
                                 attributes[:manually_managed_features] = ManuallyManagedFeaturesField if KirvanoApp.kirvano_cloud?
  ...
  enterprise_show_page_attributes = if KirvanoApp.enterprise?
                                      attrs = %i[custom_attributes limits]
                                      attrs << :manually_managed_features if KirvanoApp.kirvano_cloud?
                                      attrs << :all_features
                                      attrs
                                    else
                                      []
  ...
  enterprise_form_attributes = if KirvanoApp.enterprise?
                                 attrs = %i[limits]
                                 attrs << :manually_managed_features if KirvanoApp.kirvano_cloud?
                                 attrs << :all_features
                                 attrs
  ...
  def permitted_attributes(action)
    ...
    # Add manually_managed_features to permitted attributes only for Kirvano Cloud
    attrs << { manually_managed_features: [] } if KirvanoApp.kirvano_cloud?
