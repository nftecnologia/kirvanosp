...
export default {
  ...
  mounted() {
    const { websiteToken, locale, widgetColor } = window.kirvanoWebChannel;
  ...
  methods: {
    ...
    setLocale(localeWithVariation) {
      ...
      const { enabledLanguages } = window.kirvanoWebChannel;
    ...
    registerCampaignEvents() {
      ...
      emitter.on('execute-campaign', campaignDetails => {
        ...
        const { websiteToken } = window.kirvanoWebChannel;
