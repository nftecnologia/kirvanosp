...
export const IFrameHelper = {
  ...
  createFrame: ({ baseUrl, websiteToken }) => {
    ...
    iframe.id = 'kirvano_live_chat_widget';
    iframe.style.visibility = 'hidden';

    let holderClassName = `woot-widget-holder woot--hide woot-elements--${window.$kirvano.position}`;
    if (window.$kirvano.hideMessageBubble) {
      holderClassName += ` woot-widget--without-bubble`;
    }
    if (isFlatWidgetStyle(window.$kirvano.widgetStyle)) {
  ...
  getAppFrame: () => document.getElementById('kirvano_live_chat_widget'),
  getBubbleHolder: () => document.getElementsByClassName('woot--bubble-holder'),
  sendMessage: (key, value) => {
    const element = IFrameHelper.getAppFrame();
    element.contentWindow.postMessage(
      `kirvano-widget:${JSON.stringify({ event: key, ...value })}`,
      '*'
    );
  },
  initPostMessageCommunication: () => {
    window.onmessage = e => {
      if (
        ...
        e.data.indexOf('kirvano-widget:') !== 0
      ) {
        return;
      }
      const message = JSON.parse(e.data.replace('kirvano-widget:', ''));
  ...
  events: {
    loaded: message => {
      updateAuthCookie(message.config.authToken, window.$kirvano.baseDomain);
      window.$kirvano.hasLoaded = true;
      const campaignsSnoozedTill = Cookies.get('cw_snooze_campaigns_till');
      IFrameHelper.sendMessage('config-set', {
        locale: window.$kirvano.locale,
        position: window.$kirvano.position,
        hideMessageBubble: window.$kirvano.hideMessageBubble,
        showPopoutButton: window.$kirvano.showPopoutButton,
        widgetStyle: window.$kirvano.widgetStyle,
        darkMode: window.$kirvano.darkMode,
        showUnreadMessagesDialog: window.$kirvano.showUnreadMessagesDialog,
      ...
      if (window.$kirvano.user) {
        IFrameHelper.sendMessage('set-user', window.$kirvano.user);
      }
      ...
      if (!window.$kirvano.resetTriggered) {
    ...
    setBubbleLabel(message) {
      setBubbleText(window.$kirvano.launcherTitle || message.label);
    },

    setAuthCookie({ data: { widgetAuthToken } }) {
      updateAuthCookie(widgetAuthToken, window.$kirvano.baseDomain);
    },

    setCampaignReadOn() {
      updateCampaignReadStatus(window.$kirvano.baseDomain);
    },
  ...
  onLoad: ({ widgetColor }) => {
    ...
    iframe.setAttribute('id', `kirvano_live_chat_widget`);

    if (IFrameHelper.getBubbleHolder().length) {
      return;
    }
    createBubbleHolder(window.$kirvano.hideMessageBubble);
    onLocationChangeListener();

    let className = 'woot-widget-bubble';
    let closeBtnClassName = `woot-elements--${window.$kirvano.position} woot-widget-bubble woot--close woot--hide`;
