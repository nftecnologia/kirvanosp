...
const latestKirvanoVersion = computed(() => {
  return currentAccount.value.latest_kirvano_version;
});

const globalConfig = useMapGetter('globalConfig/get');

const hasAnUpdateAvailable = computed(() => {
  if (!semver.valid(latestKirvanoVersion.value)) {
    return false;
  }

  return semver.lt(globalConfig.value.appVersion, latestKirvanoVersion.value);
});
    ...
    <div v-if="hasAnUpdateAvailable && globalConfig.displayManifest">
      {{
        t('GENERAL_SETTINGS.UPDATE_CHATWOOT', {
          latestKirvanoVersion: latestKirvanoVersion,
        })
      }}
    </div>
