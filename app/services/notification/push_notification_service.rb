class Notification::PushNotificationService
  ...
  def perform
    ...
    notification_subscriptions.each do |subscription|
      ...
      send_push_via_kirvano_hub(subscription)
    end
  end
  ...
  def send_browser_push(subscription)
    ...
    KirvanoExceptionTracker.new(e, account: notification.account).capture_exception
    true
  end
  ...
  def send_push_via_kirvano_hub(subscription)
    return if firebase_credentials_present?
    return unless kirvano_hub_enabled?
    return unless subscription.fcm?

    KirvanoHub.send_push(fcm_options(subscription))
  end
